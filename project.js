...
function searchData() {
    ...
    resultDiv.innerHTML = ""; // 検索結果をリセット

    switch (searchKeyword) {
        case "NHK":
            displayNHKData(resultDiv);
            break;
        case "グルメ":
            displayGourmetData(resultDiv);
            break;
        case "天気":
            displayWeatherData(resultDiv);
            break;
        default:
            resultDiv.innerHTML = "<div class='no-result-box'><p class='no-result'>該当するデータが見つかりませんでした。</p></div>";
    }
}
...
function displayNHKData(resultDiv) {
    ...
    nhkData.list.g1.forEach(function(program) {
        nhkHTML += "<tr>";
        nhkHTML += "<td>" + program.title + "</td>";
        nhkHTML += "<td>" + program.start_time + "</td>";
        nhkHTML += "<td>" + program.end_time + "</td>";
        nhkHTML += "<td>" + program.area.name + "</td>";
        nhkHTML += "</tr>";
    });
    nhkHTML += "</table></div>";
...
function displayGourmetData(resultDiv) {
    ...
    gourmetData.results.shop.forEach(function(shop) {
        ...
        gourmetHTML += "<tr><td>店舗画像</td><td><img src='" + shop.photo.mobile.l + "' alt='店舗画像'></td></tr>";
        gourmetHTML += "<tr><td>ロゴ</td><td><img src='" + shop.logo_image + "' alt='ロゴ'></td></tr>";
        gourmetHTML += "<tr><td>マップ</td><td><a href='" + shop.urls.pc + "' target='_blank'>Google Map</a></td></tr>";
        gourmetHTML += "<tr><td colspan='2'><hr></td></tr>";
    });

    gourmetHTML += "</table></div>";
    resultDiv.innerHTML = gourmetHTML;
}
...
function displayWeatherData(resultDiv) {
    ...
    weatherHTML += "<p>湿度: " + weatherData.main.humidity + "%</p>";
    weatherHTML += "<p>風速: " + weatherData.wind.speed + " m/s</p>";
    weatherHTML += "</div>";
